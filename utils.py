...
class SaveData:
    ...
    def save_model(self, model, epoch):
        os.makedirs(os.path.join('Saved_Models/Dehazing_Model/model',f'{epoch}'),exist_ok=True)
        torch.save(model.state_dict(), os.path.join(self.model_dir+f"/{epoch}", 'latest_model.pt'))
        torch.save(model, os.path.join(self.model_dir+f"/{epoch}", 'model_obj.pt'))
        torch.save(epoch, os.path.join(self.model_dir+f"/{epoch}", 'last_epoch.pt'))
    ...
    def add_scalar(self, tag, value, step):
        self.writer.add_scalar(tag, value, step)        # path = self.model_dir+f'/{epoch}'
